# -*- coding: utf-8 -*-
"""
Created on Mon Apr 3 15:15:37 2023
...
class Random:
    ...
    def BoxMuller(self,Lstep):
        R = self.rand()
        X = Lstep*np.cos(4*np.pi*R)
        Y = Lstep*np.sin(4*np.pi*R)
        return X, Y
    ...
    def Categorical(self, p1=0.45,p2=0.45, p3=0.45, p4=0.45, p5=0.45, p6=0.45, p7=0.45, p8=0.45):
        if p1 < 0. or p1 > 1.:
            return 1
        if p2 < 0. or p2 > 1.:
            return 2            
        ...
        if p1 + p2  < R < p1 + p2 + p3 + p4 + p5 + p6:
            return 6
        if p1 + p2  < R < p1 + p2 + p3 + p4 + p5 + p6 + p7:
           return 7
        if p1 + p2  < R < p1 + p2 + p3 + p4 + p5 + p6 +p7 + p8:
           return 8
        else:
            return
